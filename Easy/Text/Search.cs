...
namespace Easy.Text
{
    ...
    public class Search
    {
        ...
        public void FindNext(string searchString)
        {
            ...
            int pos = searchIn.IndexOf(searchString, StringComparison.CurrentCultureIgnoreCase);

            // If we found something
            if (pos > -1)
            {
                // Select it and update index of find
                _document.Selection.SetRange(_currentIndex + pos + 1, _currentIndex + pos + searchString.Length + 1);
